...
const FilterEdge = ({ id, sourceX, sourceY, targetX, targetY, data }) => {
  ...
  return (
    <>
      ...
      {data?.label && (
        <text>
          <textPath href={`#${id}`} startOffset="50%" textAnchor="middle" className="filter-label">
            {data.label}
          </textPath>
        </text>
      )}
...
const WorkflowVisualization = ({ workflowSteps = [] }) => {
  ...
  const determineConnectorType = (currentStep, nextStep, index, totalSteps) => {
    const currentText = (currentStep.title + ' ' + currentStep.description).toLowerCase();
    const nextText = (nextStep.title + ' ' + nextStep.description).toLowerCase();
    
    // Decision/branching keywords
    const decisionKeywords = ['decision', 'choose', 'select', 'filter', 'branch', 'condition', 'if', 'check', 'validate', 'review', 'approve'];
    const parallelKeywords = ['parallel', 'concurrent', 'simultaneous', 'together', 'same time'];
    ...
    if (parallelKeywords.some(keyword => currentText.includes(keyword) || nextText.includes(keyword))) {
      return 'parallel';
    }
    ...
    if (!workflowSteps || workflowSteps.length === 0) {
      ...
      const generatedNodes = workflowSteps.map((step, index) => ({
        ...
        data: {
          ...
          description: step.description || step.content?.substring(0, 50) + '...',
        },
      }));
      ...
      console.log('Set', generatedNodes.length, 'nodes and', simpleEdges.length, 'edges');
    }
